...
import { createInitialState, currentTrip, matchingTrips } from './store.js';
...
function card(trip) { return `<article class="trip-card surface-card"><div class="trip-art">${sceneSvg(trip.scenery)}<span class="trip-badge">真实旅行 · 已完成</span><span class="style-badge">${trip.style}</span></div>
  ...
  <div class="route-strip"><span>路线概览</span>${routeSvg(trip)}</div><button class="route-link" type="button" data-pending="detail">查看完整路书 ↗</button></div></article>`; }
...
document.addEventListener('click', (event) => { const button = event.target.closest('button'); if (!button) return; if (button.dataset.action === 'reset-filters') { state.filters = createInitialState().filters; render(); } if (button.dataset.pending) showToast('这个功能将在接下来的功能提交中上线。'); });
render();
